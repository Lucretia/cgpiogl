...
procedure Point_Anim is
   ...
   Point_Size   : GL.Float32 := 1.0;
   Inc_Size     : GL.Float32 := 1.0;
   Max_Point    : constant GL.Float32 := 200.0;
   Acceleration : constant GL.Float32 := 4.5;
   ...
   begin
      ...
      Point_Size := @ + (Inc_Size * Acceleration);

      if Point_Size >= Max_Point then
         Inc_Size := -1.0;
      elsif Point_Size <= 0.0 then
         Inc_Size := 1.0;
      end if;
