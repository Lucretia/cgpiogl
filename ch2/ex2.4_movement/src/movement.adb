...
procedure Movement is
   ...
   X         : GL.Float32 := 0.0;
   X_Inc     : GL.Float32 := 0.01;
   Last_Time : Timers.Milliseconds_Long := 0;
   ...
   procedure Display (Window : Windows.Window; Current_Time_MS : Timers.Milliseconds_Long) is
      ...
      Elapsed : Timers.Milliseconds_Long := Current_Time_MS - Last_Time;
   begin
      ...
      declare
         Offset_Location : GL.Int := GL.Get_Uniform_Location (Rendering_Program, C.To_C ("offset"));
      begin
         GL.Uniform (Offset_Location, X);
      end;
...
begin
   if SDL.Initialise then
      ...
      Windows.Makers.Create
        (Win      => Prog_Window,
         Title    => Encoders.Encode ("Chapter 2: Exercise 2.4"),
